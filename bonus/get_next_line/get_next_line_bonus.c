...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/13 17:18:37 by tpetros           #+#    #+#             */
/*   Updated: 2023/02/13 17:18:39 by tpetros          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE > INT_MAX || BUFFER_SIZE <= 0)
		return (NULL);
	extra_chars[fd] = ft_all_read(fd, extra_chars[fd]);
	if (!extra_chars[fd])
		return (NULL);
	next_line = ft_stripped_line(extra_chars[fd]);
	extra_chars[fd] = ft_left_chars(extra_chars[fd]);
	return (next_line);
}
